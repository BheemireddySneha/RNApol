    ...
    for a, key_a in enumerate(sorted_residues):
        ...
        for b, key_b in enumerate(sorted_residues):
            ...
            for t in range(num_trajectories):
                corr_matrix[t] = np.corrcoef(resI[t], resJ[t])[0, 1]

            correlation[a, b] = np.mean(corr_matrix)
...
def main(args):
    ...
    log.info("Calculating dynamic cross-correlation...\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lazy-load", help="Iterate through the trajectory, loading one frame into memory at a time (memory-efficient for large trajectories)",
